...
def moment_retrieval(model, data_loader, top_n, thresh, by_frame, display_interval):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(data_loader, 1):
            ...
            visual_feat = net_input["visual_feat"]
            textual_feat = net_input["textual_feat"]
            audio_feat = net_input["audio_feat"]
            textual_mask = net_input["textual_mask"]
            audio_mask = net_input["audio_mask"]
            audio_length = net_input["audio_length"]
            start = net_input["start"]
            end = net_input["end"]
            epoch = 200

            net_output = model(visual_feat, textual_feat, audio_feat, textual_mask, audio_mask, audio_length, start, end, epoch)
...
def qualitative_moment_retrieval(model, data_loader, top_n, min_thresh, max_thresh, min_length, max_length, min_start, max_end,
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(data_loader, 1):
            ...
            net_output = model(visual_feat, textual_feat, audio_feat, textual_mask, audio_mask, audio_length, start, end, epoch=200)
            best_prop, boundary_prob, fake_prop, fake_boundary_prob = get_ensemble_result(net_output)
